...
class PPO:
    ...
    def evaluate_actions(self, states, actions) -> Tuple[torch.Tensor, torch.Tensor]:
        with torch.no_grad():
            ...
            input_ids = self.tokenizer.pad({"input_ids": input_ids}, return_tensors="pt")
...
def main():
    ...
    ppo_trained_model_score = evaluate(ppo.model, tokenizer, input_sentences, expected_output_sentences)
